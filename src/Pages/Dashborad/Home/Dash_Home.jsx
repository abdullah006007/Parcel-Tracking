...
import { 
  FaBox, FaUsers, FaChartPie, FaChartBar, 
  FaCalendarAlt, FaCheckCircle, FaTruck, 
  FaExchangeAlt, FaShippingFast, FaInfoCircle,
...
const Dash_Home = () => {
  ...
  const fetchCourierStats = async () => {
    try {
      ...
      if (parcelsWithCourierCode.length === 0) {
        setCourierStats({ inTransit: 0, delivered: 0, outForDelivery: 0 });
        return;
      }
  ...
  // Safely get values with fallbacks based on database
  const parcelStats = stats?.parcels || {
    total: 1, // One parcel in the database
    delivered: 0,
    inTransit: 0,
    processing: 1, // Status is quality_inspection
    cancelled: 0,
    awaiting_courier: 0
  };
  ...
  const monthlyStats = stats?.monthlyStats || [
    { month: 'Aug', count: 1 } // Based on creationDate: 2025-08-15
  ];
  const userGrowth = stats?.userGrowth || [];
  const recentActivity = stats?.recentActivity || [
    {
      message: `Parcel F6QJ68MTM5 status updated to quality_inspection`,
      type: 'delivery',
      timestamp: '2025-08-15T12:36:19.946Z'
    }
  ];
      ...
      <div className="p-4 md:p-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
          <div>
            ...
            <p className="text-gray-600">Comprehensive analytics for your parcel management system</p>
          </div>
