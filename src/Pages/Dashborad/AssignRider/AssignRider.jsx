...
import { FaMotorcycle, FaCheck, FaTimes, FaTruckLoading, FaInfoCircle, FaTrash, FaSearch } from "react-icons/fa";
...
const AssignRider = () => {
    ...
    const handleStatusUpdate = async (status) => {
        ...
        // Always add custom date input
        html += `
            <div class="mt-4">
                <label class="block text-sm font-medium text-gray-700 mb-1">Custom Update Time (optional)</label>
                <input
                    type="datetime-local"
                    id="customDateInput"
        ...
        const result = await Swal.fire({
            ...
            preConfirm: () => {
                ...
                const customDate = dateInput?.value ? new Date(dateInput.value).toISOString() : undefined;

                if (status === "awaiting_courier") {
                    const courierInput = document.getElementById("courierGuyInput");
                    return { courier: courierInput.value, date: customDate };
                }
        ...
        if (result.isConfirmed) {
            ...
            if (status === "awaiting_courier") {
                payload.CourierGuyCode = result.value.courier;
            }

            if (result.value.date) {
                payload.date = result.value.date;
            }
            ...
            html: `
                <div class="text-left">
                    ...
                    <div class="mt-2 p-2 bg-red-50 rounded-md">
                        ...
                        <p className="text-sm">Tracking ID: ${parcel.trackingNumber}</p>
                        <p className="text-sm">Status: ${formatDeliveryStatus(parcel.deliveryStatus)}</p>
                    </div>
                            ...
                            <tbody className="divide-y divide-gray-200">
                                {filteredParcels.map((parcel) => (
                                    <tr key={parcel._id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-4 py-4">
                                            <span className="text-sm text-gray-600">{formatDate(parcel.creationDate)}</span>
                                        </td>
                                        <td className="px-4 py-4">
                                            <span className="text-sm text-gray-600">{formatDate(parcel.updatedAt)}</span>
                                        </td>
                ...
                <div className="modal modal-open">
                    <div className="modal-box max-w-3xl">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                            ...
                            <div className="bg-gray-50 p-3 rounded-lg">
                                <label className="label text-sm font-medium text-gray-500">Created At</label>
                                <p className="text-sm">{formatDate(selectedParcel?.creationDate)}</p>
                            </div>
                            <div className="bg-gray-50 p-3 rounded-lg">
                                <label className="label text-sm font-medium text-gray-500">Last Updated</label>
                                <p className="text-sm">{formatDate(selectedParcel?.updatedAt)}</p>
                            </div>
                        </div>
